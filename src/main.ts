...
function prefix(_prefix: string, options: string[]): string[] {
...
export async function run(
  ...
  const rustcVersion = await version('rustc')
  const cargoVersion = await version('cargo')
  const clippyVersion = await version('cargo', ['clippy'])
  ...
  try {
    ...
    const execOutput = await exec.getExecOutput(
      ...
      [
        'clippy',
        '--message-format=json',
        ...actionInput.options.filter(
          opt => !opt.startsWith('--message-format')
        ),
        '--',
        ...warn,
        ...allow,
